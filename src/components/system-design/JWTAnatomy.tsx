    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-border bg-card/40 p-4">
            <h3 className="mb-2 font-mono text-xs uppercase tracking-wider text-fuchsia-400">
          ...
          <div className="rounded-xl border border-border bg-card/40 p-4 relative group">
            <h3 className="mb-2 font-mono text-xs uppercase tracking-wider text-amber-500 flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground bg-secondary/50 px-1.5 py-0.5 rounded opacity-0 transition-opacity group-hover:opacity-100">
                Editable!
              </span>
            </h3>
            <textarea
          ...
          <div className="rounded-xl border border-border bg-card/40 p-4">
            <h3 className="mb-2 font-mono text-xs uppercase tracking-wider text-cyan-accent">
              Verify Signature
            </h3>
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          A JSON Web Token (JWT) consists of three base64-url encoded parts separated by dots:
          Header, Payload, and Signature. The data in the payload is{" "}
          <strong>publicly readable</strong>. If a malicious user modifies the payload (try editing
          the JSON above!), the cryptographic signature will no longer match the hashed content,
          allowing the server to reject the tampered token.
        </p>
      </div>
    </div>
  );
