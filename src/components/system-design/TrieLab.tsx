              ...
              <div className="flex items-center gap-2">
                ...
                <motion.div
                  ...
                  className={`flex size-8 items-center justify-center rounded-lg border-2 font-mono text-sm font-bold uppercase transition-colors ${
                    isActive
                      ? "border-amber-500 bg-amber-500/20 text-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"
                      : child.isEnd
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_300px]">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
            <h3 className="mb-3 font-mono text-xs uppercase tracking-wider text-amber-500">
              Autocomplete
            </h3>
            <div className="relative">
            ...
            {searchVal && (
              <div className="mt-4">
                <h4 className="mb-2 font-mono text-xs uppercase text-muted-foreground">
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestions.length === 0 ? (
                    ...
                    suggestions.map((sug) => (
                      <span
                        key={sug}
                        className="rounded bg-amber-500/20 px-2 py-1 font-mono text-xs text-amber-500 border border-amber-500/30"
                      >
                        {sug}
                      </span>
                    ))
          ...
          <div className="rounded-xl border border-border bg-card/40 p-4 font-mono text-xs">
            <h3 className="mb-2 uppercase tracking-wider text-muted-foreground">
            ...
            <div className="flex flex-wrap gap-1">
              {words.length === 0 ? (
                ...
                words.map((w) => (
                  <span
                    key={w}
                    className="text-foreground after:content-[','] last:after:content-['']"
                  >
                    {w}
                  </span>
                ))
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          A Trie (Prefix Tree) is highly efficient for autocomplete and dictionary searches. Unlike
          Hash Maps which only do exact matches, Tries can instantly find all words sharing a common
          prefix. Searching takes <code>O(L)</code> time where <code>L</code> is the length of the
          prefix.
        </p>
      </div>
    </div>
