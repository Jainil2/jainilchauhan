...
export function MessageQueue() {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-border bg-card/60 p-4">
        <div className="flex items-center gap-6 flex-wrap">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select
              value={strategy}
              onChange={(e) => setStrategy(e.target.value as "round-robin" | "hash")}
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_2fr_1fr]">
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card/60 p-4">
          ...
          <p className="mb-4 font-mono text-xs uppercase tracking-widest text-foreground">
            Producer
          </p>
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-card/40 p-4 relative">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <span className="font-mono text-xs text-muted-foreground flex items-center gap-1">
                <Info className="size-3" /> hover msg for details
              </span>
          ...
          <div className="flex flex-col gap-3 mt-2">
            {partitions.map((partition, idx) => (
              <div
                key={idx}
                className="flex h-14 w-full flex-col overflow-hidden rounded border border-border bg-background/50"
              >
                <div className="flex items-center justify-between border-b border-border/50 bg-black/20 px-2 py-0.5">
                  <span className="font-mono text-xs text-muted-foreground">P-{idx}</span>
                  <span className="font-mono text-xs text-muted-foreground">
                    LEO: <span className="text-cyan-accent">{partition.leo}</span> | Offset:{" "}
                    <span className="text-terminal">{partition.consumerOffset}</span>
                  </span>
                </div>
                    ...
                    {partition.messages.map((msg) => (
                      ...
                      >
                        ...
                        <div className="absolute bottom-full left-1/2 z-50 mb-2 hidden -translate-x-1/2 flex-col rounded border border-border bg-card p-2 text-left shadow-xl group-hover:flex">
                          <pre className="font-mono text-xs text-muted-foreground leading-tight">
                            {`{
  "id": ${msg.id},
  "offset": ${msg.offset},
  ...
  "payload": "${msg.payload}"
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card/60 p-4 relative overflow-hidden">
          <div
            className={`mb-4 flex items-center justify-center rounded-full border p-3 transition-colors ${
              consumerStatus === "lagging"
                ? "border-destructive/30 bg-destructive/10 text-destructive shadow-[0_0_15px_rgba(239,68,68,0.3)]"
          ...
          <div className="flex flex-col items-center gap-1">
            <p
              className={`font-mono text-xs px-2 py-0.5 rounded ${consumerStatus === "lagging" ? "bg-destructive/20 text-destructive animate-pulse" : "bg-terminal/20 text-terminal"}`}
            >
            ...
            <AnimatePresence>
              {consumed.map((msg) => (
                <motion.div
                  ...
                  className="w-full truncate rounded border border-border bg-background/50 px-2 py-1 text-center font-mono text-xs text-muted-foreground"
                >
