...
export function LevenshteinLab() {
  ...
  function initTable() {
    ...
    const initialDp = Array(w1.length + 1)
      .fill(0)
      .map(() => Array(w2.length + 1).fill(null));
  ...
  return (
    <div className="space-y-6">
              ...
              <table className="border-collapse font-mono text-xs">
                <thead>
                  <tr>
                    ...
                    {w2.split("").map((c, i) => (
                      <th key={i} className="p-2 text-cyan-accent">
                        {c}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {dp.map((row, i) => (
                    <tr key={i}>
                      <th
                        className={`p-2 text-left ${i === 0 ? "text-muted-foreground" : "text-terminal"}`}
                      >
                        {i === 0 ? "ε" : w1[i - 1]}
                      </th>
                      {row.map((val, j) => {
                        ...
                        let bgClass = "";
                        if (isCurrent)
                          bgClass = "bg-amber-500/20 border-amber-500 text-amber-500 font-bold";
                        else if (isDep) bgClass = "bg-terminal/10 border-terminal/50";
                        else if (isDoneCell)
                          bgClass =
                            "bg-fuchsia-500/20 border-fuchsia-500 text-fuchsia-400 font-bold shadow-[0_0_10px_rgba(217,70,239,0.5)]";
          ...
          <div className="rounded-xl border border-border bg-card/40 p-4 font-mono text-xs">
            ...
            <ul className="flex flex-col gap-1 text-xs">
              <li className="flex items-center gap-2">
                <span className="text-amber-500">Current (i,j)</span> = min of:
              </li>
              <li className="pl-2 border-l border-border/50 ml-1 text-terminal/80">
                Top + 1 (Delete)
              </li>
              <li className="pl-2 border-l border-border/50 ml-1 text-terminal/80">
                Left + 1 (Insert)
              ...
              <li className="pl-2 border-l border-border/50 ml-1 text-terminal/80">
                Top-Left + (0 if match else 1) (Substitute)
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          The Levenshtein Distance is a classic Dynamic Programming problem used in spell checkers,
          DNA sequence analysis, and fuzzy search algorithms. It works by breaking the problem down
          into smaller subproblems, building up a matrix of the minimum edit distances for all
          prefixes of the two words.
        </p>
      </div>
    </div>
  );
