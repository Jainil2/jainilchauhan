...
export function WebAuthnLab() {
  ...
  const [serverState, setServerState] = useState<{ challenge: string; publicKey: string } | null>(
    null,
  );
  ...
  if (phase !== "idle") {
    ...
    if (
      ...
      if (phase !== "register")
        logEntries.push({
          msg: `← Challenge: ${serverState?.challenge}`,
          color: "text-fuchsia-400",
        });
      if (phase === "register_key" || phase === "register_done")
        logEntries.push({ msg: "→ Authenticator generates key pair", color: "text-cyan-accent" });
      if (phase === "register_done") {
        logEntries.push({
          msg: `→ Public key sent to server: ${serverState?.publicKey.slice(0, 16)}...`,
          color: "text-terminal",
        });
        logEntries.push({
          msg: "✓ Registration complete. Private key stays on device.",
          color: "text-terminal font-bold",
        });
      }
    }
    if (
      phase === "auth" ||
      phase === "auth_challenge" ||
      phase === "auth_sign" ||
      phase === "auth_done" ||
      phase === "auth_fail"
    ) {
      logEntries.push({
        msg: "→ Authentication: Server sends new challenge",
        color: "text-amber-500",
      });
      if (phase !== "auth")
        logEntries.push({
          msg: "← Authenticator signs challenge with private key",
          color: "text-cyan-accent",
        });
      if (phase === "auth_sign" || phase === "auth_done" || phase === "auth_fail")
        logEntries.push({ msg: "→ Signed assertion sent to server", color: "text-fuchsia-400" });
      if (phase === "auth_done")
        logEntries.push({
          msg: "✓ Server verifies signature with stored public key.",
          color: "text-terminal font-bold",
        });
      if (phase === "auth_fail")
        logEntries.push({
          msg: "✗ Signature verification FAILED. Wrong private key.",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            disabled={
              !registered ||
              (phase !== "register_done" && phase !== "auth_done" && phase !== "auth_fail")
            }
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer font-mono text-xs text-muted-foreground hover:text-foreground">
            ...
            >
              <div
                className={`absolute top-0.5 left-0.5 size-3 rounded-full bg-white transition-transform ${signWithWrongKey ? "translate-x-4" : ""}`}
              />
            </div>
            Simulate wrong key
          </label>
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_250px]">
        ...
        <div className="rounded-xl border border-border bg-card/40 p-6 min-h-[360px]">
          <div className="flex justify-between mb-8 px-4 font-mono text-xs uppercase tracking-wider text-muted-foreground">
            <div className="flex flex-col items-center gap-2">
              <div
                className={`flex size-14 items-center justify-center rounded-full border-2 transition-all duration-500 ${phase !== "idle" ? "border-cyan-accent text-cyan-accent bg-cyan-accent/10 shadow-[0_0_15px_rgba(34,211,238,0.2)]" : "border-border text-muted-foreground"}`}
              >
            ...
            <div className="flex flex-col items-center gap-2">
              <div
                className={`flex size-14 items-center justify-center rounded-full border-2 transition-all duration-500 ${phase !== "idle" ? "border-amber-500 text-amber-500 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.2)]" : "border-border text-muted-foreground"}`}
              >
          ...
          <div className="relative mx-12 space-y-4">
            ...
            {logEntries.map((entry, i) => (
              ...
              >
                ...
                <span className="shrink-0 rounded bg-background/80 border border-border/50 px-2 py-1 text-center min-w-max">
                  {entry.msg}
                </span>
                <div className="h-0.5 w-full border-b border-border/30" />
              </motion.div>
            ...
            {phase === "auth_fail" && (
              <motion.div
                initial={animate ? { x: [-8, 8, -8, 8, 0] } : false}
                animate={{ x: 0 }}
                className="flex justify-center pt-2"
              >
        ...
        <div className="space-y-4 font-mono text-xs">
          <div className="rounded-xl border border-border bg-card/40 p-4">
            ...
            <div
              className={`space-y-2 border rounded p-2 ${registered ? "border-cyan-accent/30 bg-cyan-accent/5" : "border-border bg-background/50"}`}
            >
              <div className="flex justify-between">
                <span className="text-muted-foreground">Private Key</span>
                <span className={registered ? "text-cyan-accent" : "text-muted-foreground"}>
                  {registered ? "Stored (secure)" : "—"}
                </span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="rounded-xl border border-border bg-card/40 p-4">
            ...
            <div
              className={`space-y-2 border rounded p-2 ${serverState?.publicKey ? "border-amber-500/30 bg-amber-500/5" : "border-border bg-background/50"}`}
            >
              <div className="flex justify-between">
                <span className="text-muted-foreground">Username</span>
                <span className={serverState ? "text-amber-500" : "text-muted-foreground"}>
                  {serverState ? username : "—"}
                </span>
              </div>
                ...
                <span
                  className={
                    serverState?.publicKey
                      ? "text-amber-500/80 break-all text-xs"
                      : "text-muted-foreground"
                  }
                >
                  {serverState?.publicKey ? serverState.publicKey.slice(0, 24) + "..." : "—"}
                </span>
              </div>
            </div>
            <p className="mt-2 text-xs text-muted-foreground">
              Server only stores the public key — never passwords.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          WebAuthn eliminates passwords entirely. During <strong>Registration</strong>, the
          authenticator generates a key pair on-device and sends only the public key to the server.
          During <strong>Authentication</strong>, the server sends a challenge, the authenticator
          signs it with the private key (requiring biometric confirmation), and the server verifies
          the signature. Even if the server is breached, there is no password hash to crack.
        </p>
      </div>
    </div>
  );
