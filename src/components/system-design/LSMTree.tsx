  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4 rounded-xl border border-border bg-card/40 p-4">
          <div className="flex items-center justify-between font-mono text-sm uppercase tracking-wider text-muted-foreground">
            ...
            <span className="text-xs">
              {memTable.length} / {MEMTABLE_LIMIT}
            </span>
          </div>
              ...
              ssTables.map((table) => (
                ...
                >
                  <div className="mb-1 flex justify-between font-mono text-xs uppercase text-muted-foreground">
                    ...
                    <span className={table.level === 0 ? "text-amber-500" : "text-fuchsia-400"}>
                      Level {table.level}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {table.entries.map((e) => (
                      <span
                        key={e.key}
                        className="rounded bg-secondary/50 px-1 py-0.5 font-mono text-xs text-foreground"
                      >
                        {e.key}={e.val}
                      </span>
                    ))}
          ...
          {log.map((entry) => (
            <motion.li
              ...
              className={`border-b border-border/50 pb-1 last:border-0 ${
                entry.msg.startsWith("Write")
                  ? "text-cyan-accent"
                  : entry.msg.startsWith("Flush")
                    ? "text-amber-500"
                    : "text-fuchsia-400"
              }`}
            >
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          LSM Trees provide extremely fast writes by appending to an in-memory tree (MemTable)
          instead of randomly updating disk blocks. When the MemTable fills up, it flushes
          sequentially to disk as an immutable SSTable (Sorted String Table). Background compaction
          processes merge smaller SSTables into larger ones to clean up obsolete data and optimize
          read performance.
        </p>
      </div>
    </div>
