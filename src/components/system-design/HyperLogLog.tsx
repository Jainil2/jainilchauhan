...
export function HyperLogLog() {
  ...
  const [log, setLog] = useState<
    { id: number; str: string; hashBin: string; bucket: number; zeros: number }[]
  >([]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-terminal/30 bg-terminal/5 p-4 text-center shadow-[0_0_15px_rgba(20,184,166,0.1)]">
          <h3 className="font-mono text-xs uppercase tracking-wider text-terminal/80">
        ...
        <div className="grid grid-cols-8 gap-2 sm:grid-cols-16">
          {buckets.map((val, i) => (
              ...
              animate={
                animate
                  ? {
                      scale: [0.95, 1],
                      backgroundColor:
                        val > 0 ? "rgba(20,184,166,0.2)" : "rgba(var(--secondary), 0.3)",
                    }
                  : false
              }
            ...
            >
              ...
              <span className={val > 0 ? "text-terminal font-bold" : "text-muted-foreground"}>
                {val}
              </span>
            </motion.div>
          ))}
            ...
            {log.map((entry) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded bg-secondary/50 px-1 py-0.5 text-amber-500">
                    Bucket {entry.bucket}
                  </span>
                  <span className="rounded bg-secondary/50 px-1 py-0.5 text-fuchsia-400">
                    Zeros: {entry.zeros}
                  </span>
                </div>
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          HyperLogLog estimates cardinality using extremely little memory. It hashes inputs and
          records the maximum number of leading zeros seen in each bucket. The probability of seeing{" "}
          <code>k</code> leading zeros is <code>1/2^k</code>, so the maximum zeros correlate to the
          log base 2 of the cardinality.
        </p>
      </div>
    </div>
  );
