  ...
  return (
    <div className="space-y-6">
        ...
        <ul className="space-y-2">
          {log.length === 0 && (
            <li className="text-muted-foreground">
              Perform an event to see vector clocks update...
            </li>
          )}
          {log.map((entry) => {
            const textClass =
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          Vector Clocks track causality in distributed systems where absolute time is impossible to
          determine. By looking at two vector clocks, we can tell if Event X caused Event Y, Event Y
          caused Event X, or if they were concurrent.
          <br />
          <br />
          <strong>Rule 1:</strong> On any event, increment your own index.
          <br />
          <strong>Rule 2:</strong> On receive, take the pairwise maximum of the incoming clock and
          your own, then increment your own index.
        </p>
      </div>
    </div>
  );
