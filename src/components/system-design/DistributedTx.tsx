...
export function DistributedTx() {
  ...
  useEffect(() => {
    reset();
  }, [strategy, failPayment, failInventory]);
  ...
  function getNodeColor(state: string) {
    switch (state) {
      ...
      case "preparing":
      case "committing":
        return "border-amber-500/50 bg-amber-500/20 text-amber-500";
      case "prepared":
      case "committed":
        return "border-terminal bg-terminal/20 text-terminal glow-terminal";
      case "aborted":
        return "border-destructive/50 bg-destructive/20 text-destructive";
      case "compensated":
        return "border-fuchsia-500/50 bg-fuchsia-500/20 text-fuchsia-400";
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <NodeCard
          name="Order Service"
          state={orderState}
          colorClass={getNodeColor(orderState)}
          animate={animate}
        />
        <NodeCard
          name="Payment Service"
          state={paymentState}
          colorClass={getNodeColor(paymentState)}
          animate={animate}
        />
        <NodeCard
          name="Inventory Service"
          state={inventoryState}
          colorClass={getNodeColor(inventoryState)}
          animate={animate}
        />
      </div>
        ...
        <ul className="space-y-1">
          ...
          {log.map((entry, i) => (
            <motion.li
              ...
              className={
                entry.includes("Failed") || entry.includes("Abort")
                  ? "text-destructive"
                  : entry.includes("Compensate")
                    ? "text-fuchsia-400"
                    : "text-terminal"
              }
            >
...
function NodeCard({
  ...
  return (
    ...
    >
      <span className="mb-2 font-mono text-xs font-bold uppercase tracking-wider opacity-80">
        {name}
      </span>
