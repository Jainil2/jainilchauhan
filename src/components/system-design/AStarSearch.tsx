...
export function AStarSearch() {
  ...
  const [grid, setGrid] = useState<number[][]>(() => {
    const g = Array(ROWS)
      .fill(0)
      .map(() => Array(COLS).fill(0)); // 0: empty, 1: wall
  ...
  return (
    <div className="space-y-6">
          ...
          >
            {grid.map((row, r) =>
              row.map((val, c) => {
                ...
                if (isStart)
                  bgClass = "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)] z-10 text-black";
                else if (isEnd)
                  bgClass = "bg-fuchsia-500 shadow-[0_0_8px_rgba(217,70,239,0.8)] z-10 text-white";
        ...
        <div className="flex w-40 flex-col gap-4 font-mono text-xs">
          ...
          <div className="rounded-lg border border-border bg-card/40 p-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-sm bg-muted-foreground/80"></div>Wall
              </div>
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-sm bg-terminal/20"></div>Visited
              </div>
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-sm bg-cyan-accent/30"></div>Frontier
              </div>
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-sm bg-terminal shadow-[0_0_8px_rgba(20,184,166,0.6)]"></div>
                Path
              </div>
            </div>
          </div>
      ...
      <div className="rounded-md border border-cyan-accent/20 bg-cyan-accent/5 p-4 font-mono text-xs text-cyan-accent/80">
        <p>
          <strong>Dijkstra</strong> expands in all directions equally (Breadth-First Search on an
          unweighted grid), visiting many unnecessary nodes.
          <strong>A* Search</strong> uses a heuristic (Manhattan distance) to prioritize exploring
          nodes that move <em>closer to the goal</em>, resulting in significantly fewer visited
          nodes while still finding the shortest path.
        </p>
      </div>
    </div>
  );
